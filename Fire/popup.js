...
function constructOptions (kButtonTheme) {
  for (let item of kButtonTheme) {
    ...
    button.style.backgroundImage = "url('https://raw.githubusercontent.com/KertuMikk/YoutubeColormodesExtension/master/Fire/images/" + item + ".png')"

    button.onclick = function () {
      chrome.storage.local.set({item: item + '.css'}, function () {
        console.log('color is ' + item)
      })
      browser.tabs.removeCSS({file: 'css/' + item + '.css'})
      browser.tabs.insertCSS({file: 'css/' + item + '.css'})
    }
